 ...
 *
 * Maps each field in the projection table to its detailed explanation,
...
export const CALCULATIONS = {
  ...
  cumulativeExpenses: {
    ...
    concept:
      'Running total of all expenses since projection start. Useful for understanding total spending over retirement and comparing against cumulative taxes.',
    formula:
      'cumulativeExpenses = Sum of annual expenses\n\nIncludes: Base expenses + inflation + overrides\nDoes not include taxes or IRMAA',
    backOfEnvelope: 'Sum of all years spending to date',
    compute: data => {
      ...
      return {
        formula: `Running sum of annual expenses`,
        values: `Through year ${yearsFromStart + 1}: ${yearsFromStart + 1} years of spending`,
        result: `Cumulative Expenses = ${fM(cumulativeExpenses)}`,
        simple: `avg ${fK(avgPerYear)}/yr × ${yearsFromStart + 1} years`,
      };
    },
  },
  ...
  atWithdrawal: {
    ...
    compute: data => {
      ...
      // Step-by-step breakdown for complex withdrawal logic
      const steps = [
        {
        ...
        },
        {
          ...
          values: `${fK(grossNeed)} - ${fK(ssAnnual)}`,
          result: fK(netNeed),
          fields: ['ssAnnual'],
        },
        {
          label: 'After IRA RMD',
          formula: 'remainingNeed - rmdRequired',
          values: `${fK(netNeed)} - ${fK(rmdRequired)}`,
          result: fK(afterRMD),
          fields: ['rmdRequired'],
        },
        {
          ...
          values: `min(${fK(atBOY)}, ${fK(afterRMD)})`,
